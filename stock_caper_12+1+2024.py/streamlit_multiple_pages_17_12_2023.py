...
def Loss_pie(pie_df, symbol,width,height):
    ...
    fig.update_layout(legend_title='Loss value', legend_y=0.9,paper_bgcolor= '#80aaff', width=width, height=height)
    fig.update_traces(textinfo='percent+label', textposition='inside', textfont=dict(size=14, color="white", family="Arial Black"))
    # st.plotly_chart(fig, use_container_width=True, theme=None)
    return fig
...
all_qttar_dict = {"Holding": None, "Intraday": None, "Options": None, "Charges": None,
                  "Charges_credits_debits":None,"Dividends": None,"Holding_Intraday_Options":None}
...
def all_qttar_dict_data(all_qttar_dict,addresh,file_extension):
    ...
    # all_qttar_dict["Holding_Intraday_Options"] = all_qttar_dict["Holding"]
    # all_qttar_dict["Holding_Intraday_Options"] = all_qttar_dict["Intraday"]
    # all_qttar_dict["Holding_Intraday_Options"] = all_qttar_dict["Options"]
...
def uploaded_file(uploaded_file):
    ...
    return all_qttar_dict_data(all_qttar_dict=all_qttar_dict,addresh="taxpnl-JD0585-2023_2024-Q1-Q3.xlsx",file_extension=pd.read_excel)


#  stock market journal  STOCK MARET JOURNAL   Z AI TECHNOLOGY     manojkukna/JANRAL_ZERODHA
#   STOCK MARKET JANRAL
# streamlit run streamlit_multiple_pages_16_12_2023.py
