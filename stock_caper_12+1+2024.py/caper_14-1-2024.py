...
def run():
        ...
        if app == "Home":
            ...
            # Clean the data by replacing NaN values with zeros
            percentage_change_cleaned = percentage_change.replace(np.nan, 0)

            # Create a pie chart with labels for percentage change
            plt.figure(figsize=(8, 8))
            plt.pie(percentage_change_cleaned, labels=[f"{stock}: {change:.2f}%" for stock, change in
                                                       zip(percentage_change_cleaned.index, percentage_change_cleaned)],
                    autopct='%1.1f%%', startangle=140)
            plt.title('Stock Portfolio Composition in Percentage Change')
            plt.show()
