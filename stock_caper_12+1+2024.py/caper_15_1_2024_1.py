...
def run():
        ...
        selected_stock_Sector = st.sidebar.selectbox('selected dataset for  INDEX',  NSE_all['Sector'].unique(), key='Sector')
        selected_stock_EXCHANGE = st.sidebar.selectbox('selected dataset for  EXCHANGE', ['BSE', 'NSE'], key='EXCHANGE')

        NSE_df = NSE_all.loc[( NSE_all["Sector"] == selected_stock_Sector) & (NSE_all["Exchange"] == selected_stock_EXCHANGE)]
        EXCHANGE = {"BSE": 'BO', "NSE": 'NS'}
        data_tickers = tickers_add_BO_NS(tickers=NSE_df['Ticker'].tolist(), EXCHANGE=EXCHANGE[selected_stock_EXCHANGE])
        #
        # print(data_tickers)



        start = st.sidebar.date_input('Start Date', value=pd.to_datetime('2023-01-01'), key='start')
        end = st.sidebar.date_input('End Date', value=pd.to_datetime('today'), key='end')
        top_20 = st.number_input(" top_21", value=15, key='top_21')
        selected_interval = st.sidebar.selectbox('selected dataset for selected_interval', ['1d', '2m' ,'5m','10m','15m','30m'], key='interval')
        ...
        if app == "Home":
            ...
            yfinance_download1 = yfinance_download(tickers_list=data_tickers, start_date=start, end_date=end,interval=selected_interval)
            print_sheets(df=yfinance_download1, sheets_name="Adj Close", range="A1")
            ...
            st.dataframe(live_market_data.round(2), use_container_width=False, width=1200, height=300)
...
# streamlit run caper_15_1_2024_1.py
