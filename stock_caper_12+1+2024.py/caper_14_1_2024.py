...
def run():
        ...
        if app == "Home":
            ...
            top_20 = st.number_input( " top_20", value=10, key='top_20')
            ...
            df = yf.download(data_tickers, start=start, end=end)['Adj Close']
            percentage = ((df - df.shift(1)) / df.shift(1)) * 100
            sum=(pd.DataFrame(percentage).sum())

            profitable_stocks = sum[sum > 0]

            percentage_change_series = pd.Series(profitable_stocks)
            absolute_values = percentage_change_series.iloc[-1]
            # print([f"{stock}\n{change:.2f}%\n{value:.2f}" for stock, change, value in
            #        zip( percentage_change_series.index, percentage_change_series, absolute_values)])
            # Sort the Series by values in descending order
            sorted_series = percentage_change_series.sort_values(ascending=False).head(top_20)
            print(sorted_series)

            plt.figure(figsize=(8, 8))
            plt.pie(sorted_series, labels=sorted_series.index, autopct='%1.1f%%', startangle=140)
            plt.title('Stock Portfolio Composition')
            # plt.show()

            total_values =  df.iloc[-1]
